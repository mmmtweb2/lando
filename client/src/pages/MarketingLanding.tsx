...
import {
  ...
  Check, ChevronDown, ArrowLeft, Wand2, Rocket, Menu, X, Mail,
} from 'lucide-react';
...
// hand-rolled approach already used in LandingViewer.tsx for the same reason.
    ...
    <div dir="rtl" className="relative min-h-screen font-sans text-slate-800 overflow-x-hidden" style={{ background: 'var(--bg)' }}>
      ...
      <Section id="pricing" className="py-20 bg-[#D8DFF0]">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-5 items-stretch">
          ...
          <div className="rounded-3xl border-2 p-7 shadow-lg bg-white flex flex-col" style={{ borderColor: '#DCE4F7' }}>
            ...
            <ul className="mt-6 space-y-3 text-sm flex-1">
              {['בנייה ועריכה חופשית לפני תשלום', 'דף באוויר עם כתובת משלו לשנה', '10 קרדיטי AI במתנה לעריכות', 'עד 2 יצירות דפים כלולות', 'טופס לידים ואזור אישי', 'חידוש שנתי — 99 ₪ בלבד'].map((f) => (
          ...
          <div className="rounded-3xl border-2 p-7 shadow-lg bg-white flex flex-col" style={{ borderColor: '#DCE4F7' }}>
            ...
            <ul className="mt-6 space-y-3 text-sm flex-1">
              {['5 דפים לפרסום, מתי שתרצו', 'היתרה לא פגה ולא מתחדשת חודשית', '75 קרדיטי AI במתנה', 'עד 10 יצירות דפים כלולות', 'חידוש שנתי מוזל — 79 ₪ לדף', 'בלי מנוי ובלי חיוב חוזר'].map((f) => (
                <li key={f} className="flex items-center gap-2.5"><span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: GLOW, boxShadow: '0 0 8px #6FE7FF' }} /><span className="text-slate-600">{f}</span></li>
              ))}
            </ul>
          ...
          <div className="rounded-3xl border-2 p-7 shadow-xl bg-white flex flex-col relative" style={{ borderColor: PRIMARY }}>
            ...
            <ul className="mt-6 space-y-3 text-sm flex-1">
              {['10 דפים לפרסום, מתי שתרצו', 'היתרה לא פגה ולא מתחדשת חודשית', '150 קרדיטי AI במתנה', 'עד 20 יצירות דפים כלולות', 'חידוש שנתי מוזל — 59 ₪ לדף', 'בונוס: הסרת מיתוג Pagey מהדפים, לתמיד'].map((f) => (
                <li key={f} className="flex items-center gap-2.5"><span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: GLOW, boxShadow: '0 0 8px #6FE7FF' }} /><span className="text-slate-600">{f}</span></li>
              ))}
            </ul>
            <Link to="/login" className="mt-8 block text-center rounded-xl py-3.5 font-bold text-white shadow hover:opacity-90 transition" style={{ background: GRAD }}>
        ...
        <p className="mt-8 text-center text-sm text-slate-500 max-w-2xl mx-auto">
          אין מנוי ואין חיוב חוזר — רכישת חבילה היא חד־פעמית, וזכות הפרסום שלה לא פגה ולא מתחדשת מעצמה.
          יחד עם זאת, כל דף שמתפרסם בפועל בתוקף לשנה מרגע הפרסום (כמו כל דף ב-Pagey) — ניתן לחדש אותו (לא חובה) לשנה נוספת, במחיר מוזל ככל שקניתם יותר דפים.
        </p>
      </Section>
      ...
      <Section id="faq" className="py-20 bg-[#E7EBF7]">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <FaqItem q="אני בונה הרבה דפים — יש מחיר משתלם?" a="כן, ובלי מנוי. חבילת 5 דפים עולה 990 ₪ (198 ₪ לדף) וחבילת 10 דפים 1,490 ₪ (149 ₪ לדף, וגם בלי מיתוג Pagey על הדפים לתמיד). זו רכישה חד־פעמית: כל פרסום מנכה דף אחד מהיתרה, והיתרה לא פגה. חבילה גם מוזילה את מחיר החידוש השנתי (79 ₪ לדף בחבילת 5, 59 ₪ בחבילת 10, במקום 99 ₪). רוצים רק להסיר את מיתוג Pagey בלי לקנות חבילה? אפשר לרכוש את זה בנפרד ב-350 ₪, גם על דף בודד. רוכשים מהאזור האישי." />
      ...
      {/* ── Contact strip (2026-09-14) ──────────────────────────────────────
          Elegant, prominent contact point placed right after the FAQ — the
          natural moment someone who still has an unanswered question is
          looking for a way to reach a human. Pulled out of the footer per
          Moshe's ask (was too easy to miss there) and given real brand icons
          instead of plain text links. */}
      <Section className="py-14">
        <motion.div
          initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="max-w-3xl mx-auto rounded-2xl p-8 sm:p-10 text-center"
          style={{ background: '#fff', border: '1px solid #E2E8F5', boxShadow: '0 2px 12px rgba(14,33,72,0.05)' }}>
          <h3 className="text-xl sm:text-2xl font-extrabold" style={{ color: NAVY }}>יש לכם שאלה? נתקלתם בבעיה?</h3>
          <p className="mt-2 text-sm text-slate-500">אנחנו כאן בשבילכם — אל תהססו לפנות בכל נושא, קטן כגדול.</p>
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <a
              href="mailto:tirnoer.digital@gmail.com"
              className="inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold transition hover:scale-105"
              style={{ background: `${PRIMARY}12`, color: PRIMARY }}>
              <Mail size={18} /> tirnoer.digital@gmail.com
            </a>
            <a
              href="https://wa.me/16465358055"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold text-white transition hover:scale-105"
